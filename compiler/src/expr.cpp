...
OutputExpr::OutputExpr(const uint32_t outputIndex, const uint32_t valueIndex)
...
auto
OutputExpr::outputIndex() const -> uint32_t
{
  return m_outputIndex;
}

auto
OutputExpr::valueIndex() const -> uint32_t
{
  return m_valueIndex;
}

void
OutputExpr::accept(ExprVisitor& visitor) const
{
  visitor.visit(*this);
}
